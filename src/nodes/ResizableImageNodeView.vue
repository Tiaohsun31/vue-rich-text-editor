...
function onResizeMove(e: PointerEvent) {
  ...
  if (!img) return
  const editorWidth =
    img.closest('.ProseMirror')?.clientWidth ?? img.parentElement?.clientWidth ?? startW
...
<template>
  <NodeViewWrapper class="rte-image" :style="wrapperStyle">
    <span
      class="rte-image__frame"
      :class="{ 'rte-image__frame--editable': editable, 'is-selected': selected }"
      :style="frameStyle"
    >
      <img
        ref="imgRef"
